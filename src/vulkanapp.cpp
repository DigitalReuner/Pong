...
void VulkanApp::initVulkan()
{
    ...
    createTextureImage("../textures/cat.jpg");
...
void VulkanApp::createSwapChain() {
    ...
    if (VkResult res = vkCreateSwapchainKHR(device, &createInfo, nullptr, &swapChain);res != VK_SUCCESS) {
...
VulkanApp::SwapChainSupportDetails VulkanApp::querySwapChainSupport(VkPhysicalDevice device)
{
    ...
    auto res = vkGetPhysicalDeviceSurfacePresentModesKHR(device, surface, &presentModeCount, nullptr);
...
void VulkanApp::createUniformBuffers() {
    ...
    for (auto& gameObject : gameObjects) {
        ...
        for (size_t i = 0; i < (size_t)MAX_FRAMES_IN_FLIGHT; i++) {
            createBuffer(bufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, gameObject.uniformBuffers[i], gameObject.uniformBuffersMemory[i]);
...
{
    int poolsize = MAX_FRAMES_IN_FLIGHT * MAX_OBJECTS;
...
void VulkanApp::createTextureImage(const std::string &texturePath) {
...
void VulkanApp::transitionImageLayout(VkImage image, [[maybe_unused]] VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
